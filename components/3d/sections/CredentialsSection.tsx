...
function RainbowPlatform({ position }: { position: [number, number, number] }) {
  ...
  return (
    <mesh ref={ref} position={position} castShadow>
      ...
      <meshStandardMaterial color="#FF69B4" emissive="#FF69B4" emissiveIntensity={0.3} transparent opacity={0.8} />
    </mesh>
  );
}
...
export default function CredentialsSection() {
  ...
  return (
    <group position={[SECTION_X, 0, 0]}>
      ...
      {stars.map((star, i) => (<TwinklingStar key={i} position={star.position} />))}
